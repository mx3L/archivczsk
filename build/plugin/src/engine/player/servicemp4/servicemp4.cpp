...
RESULT eServiceMP4::seekToImpl(pts_t to)
{
	...
	if (!gst_element_seek (m_gst_playbin, m_currentTrickRatio, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_KEY_UNIT,
...
void eServiceMP4::playbinNotifySource(GObject *object, GParamSpec *unused, gpointer user_data)
{
	...
	if (source)
	{
		if (g_object_class_find_property(G_OBJECT_GET_CLASS(source), "user-agent") != 0 && !_this->m_user_agent.empty())
		{
...
RESULT eServiceMP4::enableSubtitles(iSubtitleUser *user, struct SubtitleTrack &track)
{
...
RESULT eServiceMP4::disableSubtitles()
{
...
RESULT eServiceMP4::getCachedSubtitle(struct SubtitleTrack &track)
{
...
RESULT eServiceMP4::getSubtitleList(std::vector<struct SubtitleTrack> &subtitlelist)
{
